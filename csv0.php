...
<body>
  ...
  <header class="">
    ...
    <nav>
      <ul id="TipoIndicador">
        <a id="Ingresos" href="csv.php?archivo=153tingresos.csv" class="<?php
		if(1 == $pestana)
		echo "Active";
		?> col-xs-4">Ingresos</a>
        <a id="EBITDA" href="csv.php?archivo=153tebitda.csv" class="<?php
		if(2 == $pestana)
		echo "Active";
		?> col-xs-4">EBITDA</a>
        <a id="Utilidad" href="csv.php?archivo=153tutilidad.csv" class="<?php
		if(3 == $pestana)
		echo "Active";
		?> col-xs-4">Utilidad</a>
      </ul>

      <select name="Trimestre" id="Trimestre" class="cs-select cs-skin-underline">
        <option value="4T15" <?php 
			if($trimestre==4)
				echo "selected";
				
		?>><a href="http://www.elfinanciero.com.mx">4TO. TRIMESTRE 2015</a></option>
        <option value="3T15" <?php 
			
			if($trimestre==3)
				echo "selected";
				
		?>><a href="">3ER. TRIMESTRE 2015</a></option>
        <option value="2T15" <?php 
			if($trimestre==2)
				echo "selected";
				
		?>><a href="">2DO. TRIMESTRE 2015</a></option>
        <option value="1T15" <?php 
			if($trimestre==1)
				echo "selected";
				
		?> ><a href="">1ER. TRIMESTRE 2015</a></option>
      </select>
	  
	   <select name="Trimestre" id="Trimestre" class="cs-select cs-skin-underline">
	    <option value="4T15">4TO. TRIMESTRE 2015</option>
        <option value="3T15" selected>3ER. TRIMESTRE 2015</option>
	   </select>
...
while ($csvcontents = fgetcsv($handle)) {
    ...
    foreach ($csvcontents as $column) {
		if($i==0 )
		{
			...
			$lineaA = "<td class='Compania'><a href='#' class='hidden-xs'>$column</a>";
		}
		else if($i==1)
			$lineaA = $lineaA . "<a href='#' class='visible-xs'>$column</a></td>";
		else if($i==2)
	...
	if(strlen($lineaB2)>0 && strlen($lineaB3)>0)
			echo $lineaA . "<td class='Fecha hidden-xs'><a href='$lineaB2'>Nota</a>". "<a href='$lineaB3'>Reporte</a></td>" . $lineaC;
		else if(strlen($lineaB2)>0)
			echo $lineaA . "<td class='Fecha hidden-xs'><a href='$lineaB2'>Nota</a>". $lineaC;
		else if(strlen($lineaB3)>0)
			echo $lineaA . "<td class='Fecha hidden-xs'><a href='$lineaB3'>Reporte</a>". $lineaC;
		else
	...
	$i++;
		...
		if(strlen($linB2)>0 && strlen($linB3)>0)
			echo $linA . "<td class='Fecha hidden-xs'><a href='$linB2'>Nota</a>". "<a href='$linB3'>Reporte</a></td>" . $linC;
		else if(strlen($linB2)>0)
			echo $linA . "<td class='Fecha hidden-xs'><a href='$linB2'>Nota</a>". $linC;
		else if(strlen($linB3)>0)
			echo $linA . "<td class='Fecha hidden-xs'><a href='$linB3'>Reporte</a>". $linC;
		else
...
<script>
  (function() {
    [].slice.call( document.querySelectorAll( 'select.cs-select' ) ).forEach( function(el) {  
	  ...
	  $('ul#TipoIndicador li').click(function(e) 
    { 
     alert($(this).text());
    });
